...
- [x] Added `squid.conf` with a CONNECT allowlist for approved domains, including npm registry access for user-local CLI upgrades
...
- [x] Replaced reliance on upstream `ubuntu/squid` startup behavior with a repo-owned Squid proxy image in `Dockerfile.squid`
- [x] Configured the repo-owned Squid image with writable runtime/log directories and non-root `proxy` execution
- [x] Split proxy image build from proxy startup so `make egress-proxy-up` does not rebuild unconditionally

## Remaining

- [ ] Expand the Squid allowlist further if additional upstream domains are required in practice
